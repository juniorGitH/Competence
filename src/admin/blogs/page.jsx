...
export default function Page() {
  ...
  const handleEdit = (post) => {
    router.push(`/admin/blogs/update?slug=${post.slug}`)
  }

  const handleView = (post) => {
    window.open(`/blogs/view?slug=${post.slug}`, '_blank');
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 ">
        {posts && posts.map(post => (
          <div key={post._id} className="bg-white rounded-lg overflow-hidden  shadow-md  mb-4">
            <Image  src={post.imageUrl} width={500} height={300}  onClick={() =>handleView(post)} alt={post.title} className="w-full cursor-pointer object-cover h-48 mb-4" />
            <div className="p-4">
              <h2 onClick={() =>handleView(post)} className="text-xl cursor-pointer font-semibold mb-2">{post.title}</h2>
